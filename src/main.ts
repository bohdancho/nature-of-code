...
const MIN_WHEELS_ANGLE = -Math.PI / 6
const MAX_WHEELS_ANGLE = Math.PI / 6
class Car {
  ...
  draw() {
    ...
    if (keyIsDown(KEY_A)) {
      this.wheelsAngle += 0.01
    } else if (keyIsDown(KEY_D)) {
      this.wheelsAngle -= 0.02
    }
    this.wheelsAngle = minmax(this.wheelsAngle, MIN_WHEELS_ANGLE, MAX_WHEELS_ANGLE)

    this.angle += sin(this.vel) * sin(this.wheelsAngle) * -0.1
    const facing = rotateAroundY(BASE_FACING, this.angle)
    this.pos.add(facing.mult(this.vel))
